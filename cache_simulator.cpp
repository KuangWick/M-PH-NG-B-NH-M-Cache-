...
public:
    ...
    void reset() {                                                //reset(): Khởi tạo lại trạng thái của cache.
        ...
        for (size_t i = 0; i < lines.size(); ++i) {              //hits, misses, reads, writes: Thống kê các sự kiện cache hit/miss, đọc/ghi.
...
private:
    ...
    int getLRUWay(const std::vector<CacheLine> &set) const {           //getLRUWay(set): Lấy dòng ít được sử dụng nhất trong một tập hợp.
        ...
        for (int i = 0; i < set.size(); ++i) {
            if (!set[i].valid)
                return i; // Use invalid line if available
            if (set[i].lru > maxLRU) {
                maxLRU = set[i].lru;
                lruWay = i;
            }
...
void processTrace(Cache &dataCache, Cache &instructionCache, const std::vector<std::pair<int, uint32_t> > &trace, bool verbose) {
    ...
    std::cout << "  Hits = " << dataCache.hits
              << ", Misses = " << dataCache.misses
              << ", Reads = " << dataCache.reads
              << ", Writes = " << dataCache.writes
              << ", Hit Ratio = "
              << (dataCache.hits + dataCache.misses > 0
    ...
    std::cout << "  Hits = " << instructionCache.hits
              << ", Misses = " << instructionCache.misses
              << ", Reads = " << instructionCache.reads
              << ", Writes = " << instructionCache.writes
              << ", Hit Ratio = "
              << (instructionCache.hits + instructionCache.misses > 0
...
int main() {
    ...
    trace.push_back(std::make_pair(INSTRUCTION_FETCH, 0x408ED4));
    trace.push_back(std::make_pair(READ_DATA, 0x10019D94));
    trace.push_back(std::make_pair(WRITE_DATA, 0x10019D88));
    trace.push_back(std::make_pair(INSTRUCTION_FETCH, 0x408ED8));
    trace.push_back(std::make_pair(INSTRUCTION_FETCH, 0x408EDC));
    trace.push_back(std::make_pair(PRINT_STATE, 0));

    bool verbose = true;


//Tạo cache dữ liệu (dataCache) và cache lệnh (instructionCache) với các thông số cụ thể.
    Cache dataCache(NUM_SETS, DATA_WAYS);
    Cache instructionCache(NUM_SETS, INSTRUCTION_WAYS);
